function [ A,b,x ] = Tussk(k,LOAD,s)
%Gives a and B for tuss for any k using backslash
%size of array is m
 m = 5+8*k;
 %creates b
 b = zeros(m,1);
...
% creates A
 ...
 for i = 1:(2*k-1)
     ...
     if mod(i,2)==0
         %row1
         A(1+(4*i),1+(4*i))=w; A(1+(4*i),(4*i))=1;
         A(1+(4*i),5+(4*i))=-w; A(1+(4*i),4+(4*i))=-1;
         %row2
         A(2+(4*i),1+(4*i))=w; A(2+(4*i),3+(4*i))=1;A(2+(4*i),5+(4*i))=w;
         %row3
         A(3+(4*i),(4*i)+2)=1; A(3+(4*i),6+(4*i))=-1;
         %row4
         A(4+(4*i),3+(4*i))=1;
